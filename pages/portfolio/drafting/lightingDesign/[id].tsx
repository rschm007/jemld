...
import { lightingDesignContentAtom, theatreContentAtom } from "@/state/content";
...
const neighborPagesImagesAtom = atomWithStorage('portfolio-drafting_lightingDesign-neighbors', [])
...
export const ProgrammingDocPage = ({
    ...
    //@ts-ignore
    useHydrateAtoms([
        [lightingDesignContentAtom, contentData]
    ])
    const [content] = useAtom(lightingDesignContentAtom);
    const [neighborPagesImages, setNeighborPagesImages] = useAtom(neighborPagesImagesAtom);

    useEffect(() => {
        if (neighborPagesImages.length === 0) {
            getMainImageURLs(contentData, "lightingDesign")
                .then(async (res) => {
                    ...
                    <section className="mt-48 overflow-x-auto">
                        ...
                        <DynamicWrapper>
                            <NextPrevDynamicPageButtons
                                pageSlug="/portfolio/drafting/lightingDesign"
